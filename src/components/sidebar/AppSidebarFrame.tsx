...
export default function AppSidebarFrame({ children, defaultWidth = 300 }: AppSidebarFrameProps) {
  ...
  return (
    <>
      ...
      <div
        className="relative h-[calc(100%-57px)] flex-shrink-0 mb-[57px]"
        style={{ width: `${width}px` }}
      >
        <aside
          ref={sidebarRef}
          className="w-full h-full flex flex-col relative bg-white rounded-default border border-border-main/50 overflow-hidden"
        >
          {children}
        </aside>
